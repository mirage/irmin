...
module Util = struct
  ...
  let tmp_dir ~sr ~fs prefix =
    ...
    let rec f () =
      ...
      let path = Eio.Path.(fs / "/tmp" / (prefix ^ Int.to_string i)) in
...
module With_existing_store = struct
  ...
  let config ~tmp_dir ~readonly : Irmin.config =
  ...
  let () =
    Alcotest.check_raises "open V1 store in RO"
      (Irmin_pack_unix.Errors.Pack_error `Migration_needed) (fun () ->
        let repo =
          With_existing_store.S.Repo.v ~sw ~fs
            (With_existing_store.config ~tmp_dir ~readonly:true)
        in
...
let test_open_RW ~sr ~fs () =
  ...
  let repo =
    With_existing_store.S.Repo.v ~sw ~fs
      (With_existing_store.config ~tmp_dir ~readonly:false)
  in
