...
module Watch = Irmin.Private.Watch.Make (Irmin.IO.Lwt)
...
module Merge = Irmin.Merge.Make (Irmin.IO.Lwt)

(* FIXME: make this available in irmin *)
module type Metadata = Irmin.Metadata.S with type 'a merge := 'a Merge.t
module type Contents = Irmin.Contents.S with type 'a merge := 'a Merge.t

module Make_ext (IO : IO) (Obj : Config) (Ref : Config) = struct
  module Make
      (M : Metadata)
      (C : Contents)
      (P : Irmin.Path.S)
      ...
      (H : Irmin.Hash.S) =
  struct
    module Lwt = Irmin.IO.Lwt
    module AO = Append_only_ext (IO) (Obj)
    module AW = Atomic_write_ext (IO) (Ref)
    module CA = Irmin.Content_addressable (Lwt) (AO)
    module S = Irmin.Make (Lwt) (CA) (AW)
...
module KV (IO : IO) = struct
  ...
  module Make (C : Contents) =
    S.Make (M) (C) (Irmin.Path.String_list) (Irmin.Branch.String)
      (Irmin.Hash.BLAKE2B)
end
