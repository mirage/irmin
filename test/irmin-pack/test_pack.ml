...
let suite_pack name_suffix ~fs indexing_strategy
  ...
  Irmin_test.Suite.create_generic_key ~name:("PACK" ^ name_suffix)
    ~import_supported:false ~init:(init ~fs) ~store ~config ~clean ()
...
let suite ~fs =
  ...
  [
    suite_pack ~fs " { Tezos }" Index.minimal (module Irmin_tezos_conf);
    suite_pack ~fs " { Small_nodes }" Index.always (module Conf_small_nodes);
    suite_mem ~fs;
  ]
