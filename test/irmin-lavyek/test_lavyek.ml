...
let store =
  Irmin_test.store (module Irmin_lavyek.Maker) (module Irmin.Metadata.None)
...
let suite ~sw ~fs =
  let path = Eio.Path.(fs / test_db) in
  let config = Irmin_lavyek.config ~sw ~fs path in
  Irmin_test.Suite.create ~name:"LAVYEK" ~init ~store ~config ~clean ~stats ()
