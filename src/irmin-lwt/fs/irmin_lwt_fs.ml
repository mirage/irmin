...
(* Lwt-flavoured shim over Irmin 4's irmin-fs.unix backend. Each Lwt-typed
   operation forwards to its Irmin 4 counterpart through Lwt_eio.run_eio. *)

let run = Lwt_eio.run_eio
let config = Irmin_fs_unix.config
let spec = Irmin_fs_unix.spec

module Append_only (K : Irmin_lwt.Type.S) (V : Irmin_lwt.Type.S) = struct
  module M = Irmin_fs_unix.Append_only (K) (V)
...
module Atomic_write (K : Irmin_lwt.Type.S) (V : Irmin_lwt.Type.S) = struct
  module M = Irmin_fs_unix.Atomic_write (K) (V)
...
(* [irmin-fs] builds [Content_addressable] on top of [Append_only] via
   [Irmin.Content_addressable.Make]. We do the same with the Lwt-typed
   counterparts. *)

module Content_addressable = Irmin_lwt.Content_addressable.Make (Append_only)
