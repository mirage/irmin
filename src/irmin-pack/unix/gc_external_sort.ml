...
module Private = struct
  ...
  (** [sort_chunks ~arr ~chunk_sz] sorts each chunk in the bigarray [arr].
      ...
      array. The last chunk may have size less than [chunk_sz] - we don't
      require the [arr] to be sized as a multiple of [chunk_sz].
      ...
      [chunk_sz] is the number of ints that are kept in memory, and so the
      overall memory usage is something like [8 * chunk_sz] (with some overhead
      for the list.. FIXME perhaps an array would be better) *)
  ...
  (** [sort ~chunk_sz ~src ~dst] sorts the (key,value) integer data in [src] and
      places it in [dst] ([src] and [dst] must be disjoint); [chunk_sz] is the
      number of integers that are held in memory when sorting each chunk. *)
  let sort ~chunk_sz ~(src : int_bigarray) ~(dst : int_bigarray) =
    sort_chunks ~arr:src ~chunk_sz;
    merge_chunks ~src ~chunk_sz ~dst;
    ()

  (** [is_sorted ~arr] returns true iff the array is sorted; we also want the
  ...
  (** [calculate_extents_oc ~src_is_sorted ~gap_tolerance ~src ~dst] takes
      {b sorted} [(off,len)] data from [src], combines adjacent extents, and
      outputs a minimal set of (sorted) extents to [dst:out_channel]; the return
      value is the length of the part of [dst] that was filled. [gap_tolerance]
      is used to provide some looseness when combining extents: if the next
      extent starts within [gap_tolerance] of the end of the previous extent,
      then it is combined with the previous (the data in the gap, which is not
      originally part of an extent, will be counted as part of the resulting
      extent). This can reduce the number of extents significantly, at a cost of
      including gaps where the data is not actually needed. *)
  let calculate_extents_oc ~(src_is_sorted : unit) ~gap_tolerance
    ...
    let _ =
      ...
      ()
    in
    let output_dst ~off ~len =
      Gc_util.Out_channel_extra.(
...
include (
  Private :
    sig
      ...
      (** [sort ~chunk_sz ~src ~dst] sorts the [src] array of [(k,v)] pairs and
          places the result in [dst]. [src] and [dst] must be disjoint. [dst]
          must be large enough to hold the result. The data is sorted in chunks;
          [chunk_sz] is the number of ints that are kept in memory when sorting
          each chunk. *)

      val is_sorted : arr:int_bigarray -> bool
      (** [is_sorted ~arr] returns true iff the array of [(k,v)] pairs is sorted
          by [k]. This is used for testing, for example. *)
      ...
      val calculate_extents_oc :
        ...
        dst:out_channel ->
        unit
      (** [calculate_extents_oc ~src_is_sorted ~gap_tolerance ~src ~dst] uses
          the sorted reachability data in [src] and outputs extent data on
          [dst]. [gap_tolerance] specifies how much gap between two extents is
          allowed for them to be combined into a single extent. *)
    end)
