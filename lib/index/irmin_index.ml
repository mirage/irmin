...
module HT (* (IK:Irmin.Hash.S) *) (K:Irmin.Hash.S) : PERSISTANT_INDEX = struct
...
module AOI (P: PERSISTANT_INDEX_MAKER) (* (IK:Irmin.Hash.S)*) (S:AO_MAKER_RAW) (K: Irmin.Hash.S) (V: Tc.S0) = struct
    ...
    (*		 
    module X (AO:AO) (PI:PERSISTANT_INDEX with type AO.key = PI.key) =   	     
    type index = PI.index
     *)
		 
    type key = K.t

    type interal_key = AO.key
...
module Make (P: PERSISTANT_INDEX_MAKER) (AO: AO_MAKER) (RW:RW_MAKER) : S_MAKER = Irmin.Make (AOI(P)(AO)) (RW)
