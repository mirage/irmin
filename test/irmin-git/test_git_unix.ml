...
let suite ~sw ~path =
  ...
  Irmin_test.Suite.create ~name:"GIT.UNIX" ~init:(init ~sw ~path) ~store ~config
    ~clean ~stats:(stats ~sw) ()

let test_non_bare ~path () =
...
let misc ~path : unit Alcotest.test_case list =
  [ ("non-bare", `Quick, fun () -> test_non_bare ~path ()) ]
