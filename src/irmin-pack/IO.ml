...
module Unix : S = struct
  ...
  *)
  ...
  let bytes_remaning_in_page_from_offset t virt_off =
    ...
    let open Int63 in
    let open Infix in
    let page_idx = phy_off / page_len in
  ...
  let read_buffer t ~off ~buf ~len =
    (* Why are the following diffs needed?? Staging should prevent the need to
       read what's in buf. *)
    let off0 = off in
    ...
    if missing = 0 then n
      ...
      (* assert ((Int63.sub t.offset t.flushed |> Int63.to_int) >= missing); *)
