...
module Make (M : Maker) = struct
  ...
  module Stats_commit = struct
    ...
    let all_paths =
      ...
      value
      & opt (some string) None
      & info [ "all_paths" ] ~doc:"Print all paths in the tree in a file."

    let run_versioned_store ~root ~commit ~all_paths
      ...
      let* commit =
        ...
        | Some hash -> (
            ...
            | Error (`Msg m) -> Fmt.kstr Lwt.fail_with "Invalid hash %S" m)
      in
      let* () = Store.stats ~all_paths ~commit repo in
      Store.Repo.close repo

    let run ~root ~commit ~all_paths () =
      match Stat.detect_version ~root with
      | `V1 -> run_versioned_store ~root ~commit ~all_paths (module Store_V1)
      | `V2 -> run_versioned_store ~root ~commit ~all_paths (module Store_V2)

    let term_internal =
      Cmdliner.Term.(
        const (fun root commit all_paths () ->
            Lwt_main.run (run ~root ~commit ~all_paths ()))
        $ path
        $ commit
        $ all_paths)
