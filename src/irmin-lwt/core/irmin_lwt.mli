...
(** {3 [Generic_key.Maker] functor is not exposed in [irmin-lwt]}

    [Irmin.Generic_key.Maker (X)] takes four per-store sub-Makers (one for each
    of [Contents_store], [Node_store], [Commit_store], [Branch_store]) and
    produces a [Maker] keyed by whatever the user-supplied sub-Makers want —
    used by backends with non-hash keys like [Pack_key.t].

    [irmin-lwt] does not expose this functor. The only way to route
    user-supplied Lwt sub-Makers through Irmin 4 is to bridge each one to its
    Eio counterpart, which forces every store operation through a Lwt -> Eio ->
    Lwt round-trip. No application user of [irmin-lwt] needs the functor: in the
    wider Irmin codebase, only two test files invoke it directly, and backends
    like [irmin-lwt-pack] implement the [Generic_key.Maker] {b signature} by
    hand using {!Wrap_store.Make} rather than instantiating the functor.

    The {b module type} [Generic_key.Maker] above is still exposed: it is the
    public signature that backends like [irmin-lwt-pack] satisfy. Only the
    functor implementation is removed.

    Backend authors with custom keyed Lwt sub-Makers should:

    - implement their backend against [Irmin]'s direct-style
      [Generic_key.Maker (X)] (Eio-typed) and apply {!Wrap_store.Make} to lift
      the result back to the Lwt surface; or
    - follow the pattern used by [irmin-lwt-pack.Maker]: write a hand-rolled
...
module Backend : sig
  ...
  (** The modules that define a complete Irmin backend. *)
end

(** {3 [Of_backend] is not supported in [irmin-lwt]}

    Irmin's [Of_backend] takes a hand-rolled [Backend.S] (Contents store, Node
    store, Commit store, Branch store, Slice, Remote, Repo) and produces a full
    store. The [irmin-lwt] shim does not expose [Of_backend]: routing a
    user-supplied Lwt-typed [Backend.S] through Irmin 4 would require an
    extensive Lwt -> Eio adapter for every sub-store (~400 lines of bridges that
    exist nowhere else and pay a runtime cost on every operation), and no
    application user of [irmin-lwt] needs this entry point — only three internal
    Irmin packages ([irmin-git], [irmin-client], [irmin-pack-mem]) use it, none
    of which are in the [irmin-lwt] roadmap.

    Users wanting to expose a custom backend on the Lwt API should:

    - implement their backend against [Irmin]'s direct-style [Backend.S]
      (Eio-typed) and apply [Irmin.Of_backend] there, then wrap the resulting
      [Generic_key.S] back to the Lwt surface using [Wrap_store.Make]; or
    - use the higher-level [Maker] / [Generic_key.Maker] / [Storage.Make] entry
      points below, which are supported. *)
...
(** {1 Shim helpers}
    ...
    Exposed for downstream packages that wrap an Irmin 4 backend in a Lwt-typed
    surface (for instance [irmin-lwt-mem], [irmin-lwt-pack]). *)

module Lwt_to_eio = Lwt_to_eio
(** Adapter functors from Lwt-typed signatures to Irmin 4's Eio-typed
    counterparts (and back, for [merge_of_eio]). *)

module Wrap_store = Wrap_store
(** [Wrap_store.Make (S) (Schema_eio) (Inner)] takes an Eio-typed
    [Generic_key.S] [Inner] whose schema is the Eio-bridged version of the
    user's Lwt-typed [Schema.S] and produces the Lwt-typed [Store.S] surface. *)
