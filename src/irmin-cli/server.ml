...
let main ~sw ~fs ~readonly ~root ~uri ~tls ~store ~contents ~hash ~dashboard
  ...
  if fingerprint then
    ...
    let dashboard =
      ...
      | None -> None
    in
    let* server = Server.v ~sw ~fs ?tls_config ?dashboard ~uri config in
...
let main readonly root uri tls (store, hash, contents) codec config_path
  ...
  in
  Lwt_main.run
  @@ main ~sw ~fs ~readonly ~root ~uri ~tls ~store ~contents ~hash ~config_path
