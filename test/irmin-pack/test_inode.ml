...
module Context_pack : CONTEXT = struct
...
module Context_mem : CONTEXT = struct
...
module Test (Context : CONTEXT) = struct
...
module Test_pack = Test (Context_pack)
module Test_mem = Test (Context_mem)

let tests =
  [
    Alcotest.test_case "add values" `Quick (fun () ->
        Lwt_main.run (Test_pack.test_add_values ()));
    Alcotest.test_case "add values to inodes" `Quick (fun () ->
        Lwt_main.run (Test_pack.test_add_inodes ()));
    Alcotest.test_case "remove values" `Quick (fun () ->
        Lwt_main.run (Test_pack.test_remove_values ()));
    Alcotest.test_case "remove inodes" `Quick (fun () ->
        Lwt_main.run (Test_pack.test_remove_inodes ()));
    Alcotest.test_case "test concrete inodes" `Quick (fun () ->
        Lwt_main.run (Test_pack.test_concrete_inodes ()));
    Alcotest.test_case "test representation uniqueness" `Quick (fun () ->
        Lwt_main.run (Test_pack.test_representation_uniqueness_maxdepth_3 ()));
    Alcotest.test_case "test truncated inodes" `Quick (fun () ->
        Lwt_main.run (Test_pack.test_truncated_inodes ()));
    Alcotest.test_case "test intermediate inode as root" `Quick (fun () ->
        Lwt_main.run (Test_pack.test_intermediate_inode_as_root ()));
    Alcotest.test_case "mem: add values" `Quick (fun () ->
        Lwt_main.run (Test_mem.test_add_values ()));
    Alcotest.test_case "mem: add values to inodes" `Quick (fun () ->
        Lwt_main.run (Test_mem.test_add_inodes ()));
    Alcotest.test_case "mem: remove values" `Quick (fun () ->
        Lwt_main.run (Test_mem.test_remove_values ()));
    Alcotest.test_case "mem: remove inodes" `Quick (fun () ->
        Lwt_main.run (Test_mem.test_remove_inodes ()));
    Alcotest.test_case "mem: test concrete inodes" `Quick (fun () ->
        Lwt_main.run (Test_mem.test_concrete_inodes ()));
    Alcotest.test_case "mem: test representation uniqueness" `Quick (fun () ->
        Lwt_main.run (Test_mem.test_representation_uniqueness_maxdepth_3 ()));
    Alcotest.test_case "mem: test truncated inodes" `Quick (fun () ->
        Lwt_main.run (Test_mem.test_truncated_inodes ()));
    Alcotest.test_case "mem: test intermediate inode as root" `Quick (fun () ->
        Lwt_main.run (Test_mem.test_intermediate_inode_as_root ()));
  ]
