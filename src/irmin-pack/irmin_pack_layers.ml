...
module Pack_lconfig = Config.Layered
  ...
  module X = struct
    ...
    module Repo = struct
      ...
      let v config =
        ...
        let upper1 = Filename.concat root (Pack_lconfig.upper_root1 config) in
        v_layer ~v:unsafe_v_upper upper1 config >>= fun upper1 ->
        let upper0 = Filename.concat root (Pack_lconfig.upper_root0 config) in
        v_layer ~v:unsafe_v_upper upper0 config >>= fun upper0 ->
        let with_lower = Pack_lconfig.with_lower config in
        let lower_root =
          Filename.concat root (Pack_lconfig.lower_root config)
        in
        (if with_lower then
        ...
        let readonly = Pack_config.readonly config in
        let copy_in_upper = Pack_lconfig.copy_in_upper config in
        let blocking_copy_size = Pack_lconfig.blocking_copy_size config in
        {
          contents;
          node;
          commit;
      ...
      let migrate config =
        ...
        let root = Pack_config.root config in
        [
          Pack_lconfig.upper_root1;
          Pack_lconfig.upper_root0;
          Pack_lconfig.lower_root;
        ]
        |> List.map (fun name ->
