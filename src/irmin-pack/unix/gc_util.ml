...
let random_int_list ~size =
  let ints =
    ...
    |> iter_k (fun ~k (len, xs) ->
           if len >= size then xs
           else k (len + 1, Random.int ((1024 * 1024 * 1024) - 1) :: xs))
  in
  ints
...
(** A small (easily held in memory) file containing just ints; loaded via mmap;
    created via mmap (so, you need to know the number of ints upfront). *)
module Small_int_file_v1 = struct
  let load fn : int list =
    ...
    let mmap =
      Bigarray.(Unix.map_file fd Int c_layout shared [| sz |])
  ...
  let binary_search ~arr ~get ~lo ~hi ~key =
    ...
    |> iter_k (fun ~k:kont (lo, hi) ->
           ...
           | true -> (
               ...
               | _ (* when x=0 *) ->
                   Some mid (* NOTE returns the index, not the value *)))
...
(** [rm_rf path] recursively deletes all files/dirs/symlinks under path;
    symlinks are not followed. NOTE uses ocaml package fileutils, [FileUtil.rm] *)
let rm_rf _path =
  (* FIXME remove dependency on FileUtil; comment for the moment; old files won't be
     deleted *)
