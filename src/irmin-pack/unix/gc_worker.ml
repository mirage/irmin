...
module Private = struct
  ...
  let calculate_extents ~reachable_fn ~sorted_fn ~extents_fn = 
    ...
    let _          = Gc_external_sort.sort ~chunk_sz ~src:reachable.arr ~dst:sorted.arr in
    let _create_extents = 
      let oc = Stdlib.open_out_bin extents_fn in
      Gc_external_sort.calculate_extents_oc ~src_is_sorted:() ~gap_tolerance ~src:sorted.arr ~dst:oc;
    ...
    let _write_extents_to_sparse = 
      0 |> iter_k (fun ~k i -> 
          ...
          | true -> 
            ...
            Gc_sparse_file.append_region sparse ~src ~src_off:off ~len ~virt_off:off;
            k (i+2))
    in
