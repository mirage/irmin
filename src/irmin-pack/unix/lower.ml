...
module Make_volume (Io : Io.S) (Errs : Io_errors.S with module Io = Io) = struct
  ...
  let archive_seq ~upper_root ~generation ~is_first ~to_archive ~off t =
    ...
    let* () =
      if (not is_first) && is_empty t then
        ...
        Io.copy_file ~src:old_mapping ~dst:mapping
      else Ok ()
    in
    let mapping_size =
      match t with
      | Empty _ ->
          (* should this be the size of old_mapping in the above case? *)
          Int63.zero
      | Nonempty { control; _ } -> control.mapping_end_poff
    in
...
module Make (Io : Io.S) (Errs : Io_errors.S with module Io = Io) = struct
  ...
  let archive_seq_exn ~upper_root ~generation ~to_archive ~off t =
    (* Allowed in RO since there is no observable modification *)
    Errs.raise_if_error
      (let open Result_syntax in
      let* v =
        match appendable_volume t with
