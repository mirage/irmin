...
module Make_old(Store : Irmin.S) : S with type store = Store.t = struct
  ...
  let rec commit : ('ctx, Store.commit option) Schema.typ Lazy.t = 
    lazy Schema.(obj "Commit"
                   ~fields:(fun commit -> [
                         io_field "tree"
                           ~typ:(non_null Lazy.(force node))
                           ~args:[]
  ...
  and branch : ('ctx, repo_branch option) Schema.typ Lazy.t = lazy Schema.(
      obj "Branch"
        ~fields:(fun branch -> [
              field "name"
                ~typ:(non_null string)
  ...
  and store : ('ctx, Store.t option) Schema.typ Lazy.t = lazy Schema.(
      obj "Store"
        ~fields:(fun store -> [
              field "repo"
  ...
  and repo : ('ctx, Store.repo option) Schema.typ Lazy.t = lazy Schema.(
      obj "Repo"
        ~fields:(fun repo -> [
              io_field "heads"
                ~typ:(non_null (list (non_null Lazy.(force commit))))
