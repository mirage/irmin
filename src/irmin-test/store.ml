...
module Make (S: S) = struct
  ...
  let n4 ~repo =
    ...
    Graph.v t ["x", normal kv2] >>= fun kn4 ->
    Graph.v t ["b", `Node kn1; "c", `Node kn4] >>= fun kn5 ->
    Graph.v t ["a", `Node kn5]
  ...
  let test_nodes x () =
    let test repo =
      ...
      with_node repo (fun n -> Graph.v n ["x", normal kv1]) >>= fun k1 ->
      with_node repo (fun n -> Graph.v n ["x", normal kv1]) >>= fun k1' ->
      ...
      let assert_no_duplicates name node =
        let names = ref [] in
        Graph.list n node >|= fun all ->
        List.iter (fun (s, _) ->
            if List.mem s !names then Alcotest.failf "%s: duplicate!" name
            else names := s :: !names
          ) all
      in
      with_node repo (fun n -> Graph.v n []) >>= fun n0 ->

      with_node repo (fun n' -> Graph.update n n' n0 ["b"] (`Node n0))
      >>= fun n1 ->
      with_node repo (fun n' -> Graph.update n n' n1 ["a"] (`Node n0))
      >>= fun n2 ->
      with_node repo (fun n' -> Graph.update n n' n2 ["a"] (`Node n0))
      >>= fun n3 ->
      assert_no_duplicates "1" n3 >>= fun () ->

      with_node repo (fun n' -> Graph.update n n' n0 ["a"] (`Node n0))
      >>= fun n1 ->
      with_node repo (fun n' -> Graph.update n n' n1 ["b"] (`Node n0))
      >>= fun n2 ->
      with_node repo (fun n' -> Graph.update n n' n2 ["a"] (`Node n0))
      >>= fun n3 ->
      assert_no_duplicates "2" n3 >>= fun () ->

      with_node repo (fun n' -> Graph.update n n' n0 ["b"] (normal kv1))
      >>= fun n1 ->
      with_node repo (fun n' -> Graph.update n n' n1 ["a"] (normal kv1))
      >>= fun n2 ->
      with_node repo (fun n' -> Graph.update n n' n2 ["a"] (normal kv1))
      >>= fun n3 ->
      assert_no_duplicates "3" n3 >>= fun () ->

      with_node repo (fun n' -> Graph.update n n' n0 ["a"] (normal kv1))
      >>= fun n1 ->
      with_node repo (fun n' -> Graph.update n n' n1 ["b"] (normal kv1))
      >>= fun n2 ->
      with_node repo (fun n' -> Graph.update n n' n2 ["b"] (normal kv1))
      >>= fun n3 ->
  ...
  let test_commits x () =
    let test repo =
      ...
      with_node repo (fun n ->
          Graph.v n ["x", normal kv1] >>= fun kt1 ->
          Graph.v n ["a", `Node kt1] >>= fun kt2 ->
          Graph.v n ["b", `Node kt2] >|= fun kt3 ->
          kt2, kt3
        ) >>= fun (kt2, kt3) ->
      ...
      History.closure c ~min:[] ~max:[kr1] >>= fun kr1s ->
      check_keys "g1" [kr1] kr1s;

      History.closure c ~min:[] ~max:[kr2] >>= fun kr2s ->
  ...
  let test_simple_merges x () =
    ...
    let test repo =
      ...
      with_contents repo (fun t ->
          Irmin.Merge.f (P.Contents.merge v t)
      ...
      let n = P.Repo.node_t repo in
      with_node repo (fun t ->
          Irmin.Merge.(f @@ P.Node.merge n t)
      ...
      with_info 3 (fun t ~info ->
          let f = Irmin.Merge.f @@ History.merge h t ~info:(fun () -> info) in
          f ~old:(old kr0) kr1 kr2)
      >>= fun kr3 ->
      merge_exn "kr3" kr3 >>= fun kr3 ->

      with_info 4 (fun t ~info ->
          let f = Irmin.Merge.f @@ History.merge h t ~info:(fun () -> info) in
          f ~old:(old kr2) kr2 kr3)
      >>= fun kr3_id' ->
      merge_exn "kr3_id'" kr3_id' >>= fun kr3_id' ->
      check S.Hash.t "kr3 id with immediate parent'" kr3 kr3_id';

      with_info 5 (fun t ~info ->
          let f = Irmin.Merge.f @@ History.merge h t ~info:(fun () -> info) in
          f ~old:(old kr0) kr0 kr3)
      >>= fun kr3_id ->
      merge_exn "kr3_id" kr3_id >>= fun kr3_id ->
  ...
  let test_trees x () =
    let test repo =
      ...
      P.Commit.find (P.Repo.commit_t repo) head >>= fun commit ->
      let node = P.Commit.Val.node (get commit) in
      P.Node.find (P.Repo.node_t repo) node >>= fun node ->
