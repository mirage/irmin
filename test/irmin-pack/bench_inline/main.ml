...
let distribution =
  let doc =
    Printf.sprintf "Size distribution. One of: %s"
      (String.concat ", " (List.map fst distributions_list))
  in
  Cmdliner.Arg.(
    value
...
let inline_contents =
  Cmdliner.Arg.(value & flag & info [ "inline" ] ~doc:"Enable inline contents")
...
let n_reads =
  Cmdliner.Arg.(
    ...
    & opt int 1_000
    & info [ "reads" ] ~docv:"N"
        ~doc:"Number of read operations for latency measurement")
...
let run_all_flag =
  Cmdliner.Arg.(value & flag & info [ "all" ] ~doc:"Run all distributions")

let csv_header =
  Cmdliner.Arg.(value & flag & info [ "header" ] ~doc:"Print CSV header")
...
let cmd_run =
  let run csv_header dist_name distribution n_contents inline_contents
      inline_threshold n_reads runs all =
    if all then run_all n_contents inline_threshold n_reads runs
    else
      run_single csv_header dist_name
        (config distribution n_contents inline_contents inline_threshold n_reads
           runs)
  in
...
let sweep_threshold =
  Cmdliner.Arg.(
    ...
    & opt int 16
    & info [ "threshold"; "t" ] ~docv:"BYTES"
        ~doc:"Inlining threshold to test around")
    ...
    List.iter
      (fun inline_contents ->
        ...
        let ( store_size,
              ...
              _non_inlined,
              _sizes ) =
          Bench.run ~fs:env#fs ~config
        in
        Printf.printf "%d,%b,%d,%.2f,%.2f,%.2f\n" size inline_contents
          store_size write_time p50 p99)
      [ false; true ]
  done
