...
let _ = 
  try
    let open struct
      ...
      let _ = Printf.printf "%s: running with pid %d\n%!" __FILE__ (Unix.getpid ())
      ...
      let _ = Printf.printf "%s: context_path=%s; commit_hash=%s; output_path=%s\n%!" __FILE__ context_path commit_hash_s output_path
      ...
      let repo = 
        Printf.printf "%s: opening repo %s\n%!" __FILE__ context_path;
        S.Repo.v config
      ...
      let iter = 
        repo >>= fun repo -> 
        commit >>= fun commit -> 
        Printf.printf "%s: calling Repo.iter\n%!" __FILE__;
        S.Commit.key commit |> fun commit_key ->
        (* Repo.iter takes callbacks for each particular type of object; the callbacks
           typically take a key; we want to ensure that each particular object is at least
           read; so for each callback we use the key to pull the full object *)
      ...
      let _ = Printf.printf "%s: after Repo.iter\n%!" __FILE__

      (* Pack store reads are logged to the output file; we don't have a handle on the
         [out_channel], so we can't directly close the channel; likely the channel is
         flushed and closed on termination anyway, but just to make sure, we flush all out
         channels at this point, just before termination *)
      let _ = Stdlib.flush_all ()
    end
    in ()
  with e -> (
      Printf.printf "%s: exception raised whilst executing main body\n" __FILE__;
      Printf.printf "%s: exception string representation: %s\n%!" 
        __FILE__ (Printexc.to_string e);
      (* make sure to finish with an "error" exit code; alternatively reraise the
         exception *)
      Stdlib.exit (-1))
