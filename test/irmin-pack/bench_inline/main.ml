...
let config distribution n_contents inline_contents n_reads runs =
  Bench.{ distribution; n_contents; inline_contents; n_reads; runs }
...
let run_all n_contents n_reads runs =
  Logs.set_level None;
  ...
  List.iter (fun (dist_name, distribution) ->
    List.iter (fun inline_contents ->
      let config = Bench.{ distribution; n_contents; inline_contents; n_reads; runs } in
      let result = Bench.run ~fs:env#fs ~config in
      Bench.print_csv_row ~dist_name ~config result
    ) [ false; true ]
  ) distributions
...
let cmd_run =
  let run csv_header dist_name distribution n_contents inline_contents n_reads runs all =
    if all then
      run_all n_contents n_reads runs
    else
      run_single csv_header dist_name (config distribution n_contents inline_contents n_reads runs)
  in
  ...
  Cmdliner.Cmd.v
    ...
    Cmdliner.Term.(const run $ csv_header $ distribution_name $ distribution
                   $ n_contents $ inline_contents $ n_reads $ runs $ run_all_flag)
...
let cmd_sweep =
  ...
  Cmdliner.Cmd.v
    ...
    Cmdliner.Term.(const run_sweep $ n_contents $ n_reads $ runs $ threshold $ sweep_range)

let cmds = [ cmd_run; cmd_sweep ]
