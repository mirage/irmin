...
module Test_reconstruct = struct
  ...
  let test_reconstruct ~fs () =
    ...
    setup_test_env ~fs ();
    let conf = config ~readonly:false ~fresh:false (root_v1 ~fs) in
    (* Open store in RW to migrate it to V3. *)
    Eio.Switch.run @@ fun sw ->
    let repo = S.Repo.v ~sw ~fs conf in
...
module Test_corrupted_stores = struct
  ...
  let test ~fs () =
    setup_env ~fs ();
    Eio.Switch.run @@ fun sw ->
    let rw = S.Repo.v ~sw ~fs (config ~fresh:false (root ~fs)) in
    [%log.app
  ...
  let test_minimal ~fs () =
    ...
    in
    Eio.Switch.run @@ fun sw ->
    let rw = S.Repo.v ~sw ~fs config in
...
module Test_traverse_gced = struct
  ...
  let test_traverse_pack ~fs ~domain_mgr () =
    ...
    S.test_traverse_pack_file ~sw ~fs `Check_index conf
end
