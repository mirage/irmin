...
(** Callback to call on each node creation *)
...
val set : ('a, 'b) callback -> ('a, 'b) t -> 'a list -> 'b -> ('a, 'b) t
(** Associates a value with the given path, or replaces if there was
    already one. The [callback] function is called on each newly
    created node.  *)

val add : ('a, 'b) callback -> ('a, 'b) t -> 'a list -> 'b -> ('a, 'b) t
(** Associates a value with the given path, keeping previous bindings *)

val set_lazy : ('a, 'b) callback -> ('a, 'b) t -> 'a list -> 'b Lazy.t -> ('a, 'b) t
(** The same but taking a lazy value *)

val unset : ('a, 'b) callback -> ('a, 'b) t -> 'a list -> ('a, 'b) t
...
val map_filter_values : ('a, 'c) callback ->
  ('b -> 'c option) -> ('a, 'b) t -> ('a,'c) t
...
val append : ('a, 'b) callback -> ('a, 'b) t -> ('a list * ('a, 'b) t) -> ('a, 'b) t
