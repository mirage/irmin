...
module Make (P: Ir_s.PRIVATE) = struct
  ...
  module Contents = struct
    ...
    let merge: t Ir_merge.t =
      let f ~old x y =
        ...
        v x >>= fun x ->
        v y >>= fun y ->
        Ir_merge.(f P.Contents.Val.merge) ~old x y >|= function
  ...
  module Node = struct
    ...
    let add t step = function
      ...
      | `Node _ | `Contents _ as v ->
        ...
        | Some m ->
          ...
          match previous with
          | None   -> Lwt.return (of_map (StepMap.add step v m))
          | Some p ->
            value_equal v p >|= function
            | true -> t
            | false -> of_map (StepMap.add step v m)
    ...
    let rec merge () =
      let f ~old x y =
        ...
        let m = StepMap.merge value_t (fun _step ->
            Ir_merge.option (merge_value ())
          ) in
        Ir_merge.(f @@ option m) ~old x y
        >|= function
    ...
    and merge_value () =
      ...
      in
      Ir_merge.seq [
        Ir_merge.default value_t;
        Ir_merge.v value_t f
      ]
  ...
  let getv (t:tree) path =
    Log.debug (fun l -> l "Tree.getv %a" Path.pp path);
  ...
  let get t k = getm t k >|= fun (c, _) -> c
  ...
  let remove t k =
    ...
    | Some (path, file) ->
      let rec aux view path =
        match Path.decons path with
        | None        -> Node.remove view file
        | Some (h, p) ->
          Node.findv view h >>= function
          | None | Some (`Contents _) -> Lwt.return view
          | Some (`Node child) ->
  ...
  let addv t k v =
    Log.debug (fun l -> l "Tree.addv %a" Path.pp k);
    match Path.rdecons k with
    | None              -> (equal t v >|= function true  -> t | false -> v)
    | Some (path, file) ->
      let rec aux view path =
        match Path.decons path with
        | None        -> Node.add view file (with_setm v)
        | Some (h, p) ->
          Node.findv view h >>= function
  ...
  let add t k ?metadata c =
    ...
    | Some (path, file) ->
      let rec aux view path =
        match Path.decons path with
        | None        -> Node.add view file (with_optm metadata c)
        | Some (h, p) ->
          Node.findv view h >>= function
          | None | Some (`Contents _) ->
            aux Node.empty p >>= fun child ->
  ...
  let merge: tree Ir_merge.t =
    let f ~old (x:tree) y =
      ...
      | Ok (`Contents (c, m)) ->
        ( Contents.v c >>= function
            | None   -> Ir_merge.conflict "conflict: contents"
            | Some c -> Ir_merge.ok (`Contents (c, m)) )
      | Ok (`Node _ as n) -> Ir_merge.ok n
