...
module type S = sig
  ...
  val notify : t -> key -> value option -> unit io
  ...
  val watch_key :
    t -> key -> ?init:value -> (value Diff.t -> unit io) -> watch io
  (** Watch a given key for changes. More efficient than {!watch}. *)

  val watch :
    t ->
    ?init:(key * value) list ->
    (key -> value Diff.t -> unit io) ->
    watch io
...
module type Watch = sig
  ...
  module type S = S with type 'a io := 'a io
  ...
  type hook = int -> string -> (string -> unit io) -> (unit -> unit io) io
