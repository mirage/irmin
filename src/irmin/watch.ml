...
module type S = sig
  ...
  val watch: t -> ?init:(key * value) list -> (key -> value Diff.t -> unit Lwt.t) ->
    watch Lwt.t
...
module Make
    (K: sig type t val t: t Type.t end)
    (V: sig type t val t: t Type.t end) =
struct
  ...
  module KMap =
    Map.Make(struct type t = K.t let compare = Type.compare K.t end)
