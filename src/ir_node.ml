...
module Make (K_c: Ir_s.S0) (K_n: Ir_s.S0) (P: Ir_s.PATH) (M: Ir_s.METADATA) =
struct
...
struct
  ...
  (* [Ir_merge.alist] expects us to return an option. [C.merge] does
  ...
  let merge_parents merge_key =
    Ir_merge.alist step_t S.Key.t (fun _step -> merge_key)
  ...
  let merge_value (c, _) merge_key =
    ...
    in
    let merge =
      Ir_merge.pair (merge_contents_meta c) (merge_parents merge_key)
    in
    Ir_merge.like S.Val.t merge explode implode
